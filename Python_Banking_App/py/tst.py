                ...
                for row in reader:
                    if len(row) >= 7: # ensure all colunms are present, was 6 before but chenged to 7 to include 'active', and used column 6 for overdraft_count
                        self.accounts.append({
                            ...
                            'name': row[1],
                            'password': row[2],
                            'checking_account': int(row[3]) if row[3] == 0 else " ", # convert from int to empty string
                            'savings_account': int(row[4]) if row[4] == 0 else " ", # convert from int to empty string
    ...
    def transfer_money(self, account):
        ...
        if inp1 == 'myaccounts':
            ...
            if inp == 'checking' or inp == 'savings':
                ...
                if account['checking_account'] == '' or account['savings_account'] == '':
        ...
        if inp1 == 'account':
            ...
            if amount <= 0:
                print("Amount must be positive.")
                return
            ##check to see if they have a checking account
            if account['checking_account'] == " ":
                ##tell them it is going to go to their other account
                ##perform the logic/math to move money to and from account
                
                ##If you have methods to withdraw and deposit from each account, it could be easy to target each instance and call the methods
                ##otherwise just do it by hand here.
                print("The Account doesn't have a Checking Account, the money will be transfered into the Savings Account.")
                inp3 = 'savings'
            ##check to see if they have a savings account
            elif account['savings_account'] == "  ":
                ##tell them it is going to go to their other account
                ##perform the logic/math to move money to and from account
                print("The Account doesn't have a Savings Account, the money will be transfered into the Checking Account.")
                inp3 = 'checking'
